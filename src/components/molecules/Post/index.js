...
export const Post = ({ post, _hover, isDetail = false }) => {
  ...
  const openImageSource = (event) => {
    event.stopPropagation();
    if (post.data.image) {
      window.open(post.data.image, "_blank");
    }
  };
  ...
  return (
    ...
    >
      ...
      {post.data.image && (
        <Flex justify="center" h={!isDetail ? "45vh" : "100%"}>
          <Image
            align="center"
            src={post.data.image}
            alt="Post Image"
            ...
            objectFit="cover"
            overflow="hidden"
            onDoubleClick={openImageSource} // Added event handler for double-click
            cursor="pointer" // Added cursor style for indication
          />
        </Flex>
      )}
